...
class Rules:
    ...
    def __init__(self, clf, important_features):
        ...
        self.print_all(important_features)
    ...
    def print_all(self, important_features):
        ...
        for i, array in enumerate(self.all_path):
            ...
            rule = (self.predict[i], sum(self.tree.value[array[0]][0]),
                    self.tree.value[array[0]][0][self.predict[i]] / sum(self.tree.value[array[0]][0]),
                    ' && '.join(conds[::-1]))

            self.rules.append(rule)

        self.rules = sorted(self.rules, key=lambda r: r[1], reverse=True)
